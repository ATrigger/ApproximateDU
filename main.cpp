...
plot3D generate3DPlotEuler() {
    ...
    for (double x = a + h; x <= b; x += h) {
        ...
        double ny1 = plot.second.first.back() + h * f[0](x, plot.second.first.back(), plot.second.second.back()),
                ny2 = plot.second.second.back() + h * f[0](x, plot.second.first.back(), plot.second.second.back());
...
plot3D generate3DPlotRGK() {
    ...
    for (double x = a + h; x <= b; x += h) {
        ...
        double k[4][2];
        for (int i = 0; i < 2; i++) {
            k[0][i] = f[i](x, oldy[0], oldy[1]);
            k[1][i] = f[i](x + h / 2, oldy[0] + (h * k[0][i] / 2), oldy[1] + (h * k[0][i]) / 2);
            k[2][i] = f[i](x + h / 2, oldy[0] + (h * k[1][i] / 2), oldy[1] + (h * k[1][i]) / 2);
            k[3][i] = f[i](x + h, oldy[0] + (h * k[2][i]), oldy[1] + (h * k[2][i]));
        }
        double ny1 = oldy[0] + (h / 6) * (k[0][0] + 2 * k[1][0] + 2 * k[2][0] + k[3][0]);
        double ny2 = oldy[1] + (h / 6) * (k[0][1] + 2 * k[1][1] + 2 * k[2][1] + k[3][1]);
        out << "x = " << x << " : y1 = " << ny1 << ", y2 = " << ny2 << std::endl;
        plot.second.first.push_back(
                ny1
        );
